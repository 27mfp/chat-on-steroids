...
export function createWorkspaceTerminal() {
  ...
  const create = async (): Promise<void> => {
    ...
    const term = new Terminal({ theme: { background: colors.getPropertyValue('--page').trim(), foreground: colors.getPropertyValue('--ink').trim() }, cursorBlink: true, fontSize: 13, fontFamily: 'Cascadia Code, Consolas, monospace', scrollback: 5000, allowProposedApi: false });
  ...
  window.addEventListener('beforeunload', () => { observer.disconnect(); stopEvents(); for (const tab of tabs.values()) tab.term.dispose(); }, { once: true });
  paint();
