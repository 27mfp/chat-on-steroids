...
export function initSkills(options: Options) {
  ...
  const choose = (skill: typeof choices[number]): void => {
    if ('command' in skill) {
      ...
      const authored = (prefixOwner === options.owner() ? displayedPrefix : '') + (skill.command === 'add' ? `Please add the following skills to my COS skills:\n${text}` : text);
      close(); project(authored); if (skill.command !== 'add') options.command?.(skill.command); input.focus(); return;
    }
  ...
  const paintInline = (): void => {
    ...
    for (const [index, skill] of choices.entries()) {
      ...
      row.title = command ? (skill.command === 'add' ? t('Add Skills') : `/${skill.command}`) : skill.path;
    ...
    const add = control('', 'skill-add'); add.append(icon('i-plus')); add.title = t('Add Skills'); ui(add, 'aria-label', () => t('Add Skills'));
    add.addEventListener('pointerdown', event => event.preventDefault());
    add.addEventListener('click', () => chooseCurrent({ command: 'add', name: 'Add Skills', description: '', glyph: 'i-plus' }));
    const footer = el('div', 'skill-menu-footer'); footer.append(add); host.append(footer);
