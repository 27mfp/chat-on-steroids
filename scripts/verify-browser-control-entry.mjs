...
const source=await fs.readFile('extension/background.js','utf8');
...
const server=http.createServer(async(req,res)=>{
  ...
  if(route==='/fixture'){res.writeHead(200,{'content-type':'text/html'});res.end('<title>Production entry fixture</title><h1>Existing tab from actual background worker</h1><button>Fixture action</button>');return;}
...
const extension=path.join(run,'extension');await fs.cp(path.resolve('extension'),extension,{recursive:true});
...
try {
  ...
  connection.on('message',raw=>{const m=JSON.parse(raw),p=pending.get(m.id);if(p){pending.delete(m.id);m.error?p.reject(new Error(m.error.message)):p.resolve(m.result);}});
  const worker=await until(async()=>(await cdp('Target.getTargets')).targetInfos.find(t=>t.type==='service_worker'&&t.url.endsWith('/background.js')),'Production worker startup');
  const extensionOrigin=new URL(worker.url).origin.replace('null',worker.url.slice(0,worker.url.lastIndexOf('/')));
  const popup=await cdp('Target.createTarget',{url:`${extensionOrigin}/popup.html`});
  ...
  if(expectUnavailable){
    ...
    const tab=await until(async()=>{const r=await broker.execute('browser_tabs',{action:'list'},'session:entry-fixture',null,async()=>true);return r.value?.tabs?.find(t=>t.url===base+'/fixture');},'Browser tab listing');
